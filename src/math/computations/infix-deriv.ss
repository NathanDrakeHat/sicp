...
(define (infix-deriv expr var)
  (cond [(number? expr) 0]
        ...
        [(sum? expr)
          (make-sum (infix-deriv (addend expr) var)
                   (infix-deriv (augend expr) var))]
        [(product? expr)
          (make-sum
            (make-product (multiplier expr)
                         (infix-deriv (multiplicand expr) var))
            (make-product (infix-deriv (multiplier expr) var)
                         (multiplicand expr)))] 
        [(exponentiaion? expr) 
          (make-product (exponent expr) 
            (make-product [make-exponentiaion (base expr) (- (exponent expr) 1)] [infix-deriv (base expr) var]))]
        [else
